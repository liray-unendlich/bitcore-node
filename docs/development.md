...
To develop phorecore-node:

```bash
cd ~
git clone git@github.com:<yourusername>/phorecore-node.git
git clone git@github.com:<yourusername>/phorecore-lib.git
```
...
We now will setup symlinks in `phorecore-node` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf phorecore-lib
ln -s ~/phorecore-lib
...
cd devnode
mkdir node_modules
touch phorecore-node.json
touch package.json
```

Edit `phorecore-node.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../phorecore-node/bin/phorecore-node start
```