...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './phorecore-node.json');
...
function lookInModuleManifest(req, service) {
  try {
    ...
    if (servicePackage.phorecoreNode) {
      serviceModule = serviceModule + '/' + servicePackage.phorecoreNode;
...
function loadModule(req, service) {
  ...
  //package.json file, if so, then see if there is a phorecoreNode field, if so
  ...
  if (!serviceCode) {
    throw new Error('Attempted to load the ' + service.name + ' service from: ' +
      ...
      process.cwd() + '/package.json" - phorecoreNode field. All paths failed to find valid nodeJS code.');
  }
