...
var phorecore = require('phorecore-lib');
var $ = phorecore.util.preconditions;
var _ = phorecore.deps._;
var utils = require('../utils');

/**
 * Will remove a service from phorecore-node.json
...
/**
 * Will remove the Node.js service and from the phorecore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The phorecore-node configuration directory
...
function remove(options, done) {
  ...
  var phorecoreConfigPath = path.resolve(configPath, 'phorecore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(phorecoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a phorecore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to phorecore-node.json
        removeConfig(phorecoreConfigPath, service, next);
      });
    }, done
  );
}
