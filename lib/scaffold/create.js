...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'phorecore-lib': '^' + phorecore.version,
    'phorecore-node': version
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/phorecore-node.json', configJSON);
      if (!isGlobal) {
 ...
 * @param {String} options.dirname - The name of the phorecore node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the phorecore-node directory and configuration
