...
/**
 * Will return the path and default phorecore-node configuration. It will search for the
 * configuration file in the "~/.phorecore" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.phorecore');
  var defaultConfigFile = path.resolve(defaultPath, './phorecore-node.json');
  ...
  if (fs.existsSync(defaultConfigFile)) {
    ...
    var now = new Date();
    // phorecore-node.YYYY-MM-DD.UnixTimestamp.json
    var backupFileName = `phorecore-node.${now.getUTCFullYear()}-${now.getUTCMonth()}-${now.getUTCDate()}.${now.getTime()}.json`;
